...
class ResultSaver:
    ...
    def _save_config(self):
        d = {
            ...
            for k in [
                "dataset", "num_owners", "num_rounds", "local_epochs",
                "hidden_dim", "num_layers", "dropout", "lr_classifier",
                "weight_decay", "num_pred", "gen_hidden_dim", "lr_generator",
                "gen_rounds", "hide_portion", "seed", "device",
                "train_ratio", "val_ratio", "fedavg_weight",
            ]
        }
    ...
    def _save_summary(self):
        ...
        for k, v in self.summary.items():
            if isinstance(v, dict):
                ...
                s[k] = [
                    {kk: round(vv, 4) if isinstance(vv, float) else vv for kk, vv in item.items()}
    ...
    def _save_report(self):
        lines = [
            ...
            f"  FedAvg: {self.cfg.fedavg_weight}  hide_h: {self.cfg.hide_portion}",
            "",
        ]
    ...
    def _save_training_curves(self):
        ...
        colors = {
            "GlobSage": "#2ecc71", "LocSage": "#e74c3c",
            "FedSage": "#3498db", "FedSage+": "#9b59b6",
        }
        fig, ax = plt.subplots(figsize=(10, 6))
        for n, d in exps.items():
            if d["a"]:
                ax.plot(d["r"], d["a"], label=n, color=colors.get(n, "#333"), linewidth=2)
